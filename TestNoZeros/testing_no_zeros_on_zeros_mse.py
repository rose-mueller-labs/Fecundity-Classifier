...
with open("no_0_training_vs_CD_0_testing.csv", "w", newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['ImagePath', 'Bot', 'Human'])
    for file in os.listdir(ROOT_DIR):
        if 'eggs' not in file or 'unsure' in file or 'eggs0' not in file:
            continue
        label = 0
...
df = pd.read_csv('no_0_training_vs_CD_0_testing.csv')
...
plt.tight_layout()
plt.plot()
plt.savefig("NO_ZEROS_no_0_training_vs_CD_0_testing")